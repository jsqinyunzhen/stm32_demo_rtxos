...
*/
void get_dbgmcu_value(void)
{
	uint32_t idcode;
	uint32_t cr;
	idcode = DBGMCU_DBG->IDCODE;
	cr = DBGMCU_DBG->CR;
	printf("DEVID=0x%X,revid=0x%x cr=0x%x\r\n",idcode&0XFFF,(idcode>>16),cr);
}

void set_dbgmcu_value(uint8_t  bitset)
{
	...
	DBGMCU_DBG->CR = cr;
	cr = DBGMCU_DBG->CR;
	printf("cr=0x%x cr[%d]=%d\r\n",cr,bitset,(cr >>bitset)&0x1);
}
/**
  * @}
  */
