...
/* Private variables--------------------------------------------------------- */
uint8_t ClockSource;
const uint8_t *MonthsNames[]= {"Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"};

//uint8_t *WeekNames[]={"Sun","Mon","Tue","Wed","Thu","Fri","Sat"};
uint8_t *WeekNames[]= {"Mon","Tue","Wed","Thu","Fri","Sat","Sun"};
#define TONE_OFFSET 8
...
//   RCC_APB1PeriphClockCmd(RCC_APB1Periph_PWR | RCC_APB1Periph_BKP, ENABLE);
    ...
    bkp_dr1 = BKP_ReadBackupRegister(BKP_DR1);
    printf("bkp_dr1=0x%04x\r\n",bkp_dr1);
// if(bkp_dr1==CONFIGURATION_RESET)
    if(bkp_dr1 != CONFIGURATION_DONE)
    {
        printf("init rtc configure\r\n");
...
//      EE_Format();
        ...
        printf("RTC_ITConfig RTC_IT_SEC...\r\n");
...
#elif defined(RTCClockSource_LSE)
            RTC_SetPrescaler(32765 /*32767*/);            /* RTC period = RTCCLK/RTC_PR = (32.768 KHz)/(32767+1) */
#endif
...
//    }
    ...
    else
    {
        ...
        printf("read rtc configure\r\n");
    }

#ifdef RTCClockOutput_Enable
    printf("RTCClockOutput_Enable\r\n");
    ...
    printf("BKP date:%d,%d,%d\r\n", s_DateStructVar.Year, s_DateStructVar.Month, s_DateStructVar.Day);
    printf("BKP Alarm date:%d,%d,%d\r\n", s_AlarmDateStructVar.Year, s_AlarmDateStructVar.Month, s_AlarmDateStructVar.Day);
}
...
//  /* Enable the EXTI Line17 Interrupt */
    ...
    printf("SetAlarm c=%d,n=%d\r\n",RTC_GetCounter(),CounterValue);
    RTC_ClearITPendingBit(RTC_IT_ALR);  //?????
    RTC_ClearFlag(RTC_FLAG_SEC);
    RTC_ClearFlag(RTC_FLAG_ALR);


    RTC_SetAlarm(RTC_GetCounter() + CounterValue);
    /* Wait for RTC registers synchronization */
    //RTC_WaitForSynchro();
...
void SetDate_Alarm(uint8_t flag)
{
    AlarmDate = flag;
}
...
//    LCD_DisplayString(Line6,Column1,"PLEASE RECONFIGURE");
    ...
    else
    {
        ...
        printf("SetDate date:%d,%d,%d\r\n", s_DateStructVar.Year, s_DateStructVar.Month, s_DateStructVar.Day);
        printf("SetDate Alarm date:0x%x,%x,%x\r\n", s_AlarmDateStructVar.Year, s_AlarmDateStructVar.Month, s_AlarmDateStructVar.Day);
...
//  CurrentWeekDay = CurrentWeekDay % 7;
    ...
    CurrentWeekDay = (CurrentDay + 2 * CurrentMonth + 3 * (CurrentMonth + 1) / 5 + CurrentYear + CurrentYear / 4 - CurrentYear / 100 + CurrentYear / 400+1) % 7;
...
void DisplayDate(void)
{
    ...
    s_DateStructVar.Day=(BKP_ReadBackupRegister(BKP_DR3));
#if 1
    printf("Y %d M %d D %d ",s_DateStructVar.Year,s_DateStructVar.Month,s_DateStructVar.Day);
#else
    ...
    weekday = WeekDay(s_DateStructVar.Year,s_DateStructVar.Month,s_DateStructVar.Day);
    printf("weekday is %d:%s \r\n",weekday,WeekNames[weekday]);
...
void DisplayTime(void)
{
    ...
    printf("Time h: %d\r\n", s_TimeStructVar.HourHigh*10+s_TimeStructVar.HourLow);
    printf("Time m: %d\r\n", s_TimeStructVar.MinHigh*10+s_TimeStructVar.MinLow);
    printf("Time s: %d\r\n", s_TimeStructVar.SecHigh*10+s_TimeStructVar.SecLow);
...
{
    uint8_t DaysElapsed;
    uint32_t check_time = RTC_GetCounter() / SECONDS_IN_DAY;
    printf("**check_time=%d\r\n",check_time);
    if(check_time != 0)
    {
        for(DaysElapsed = 0; DaysElapsed < check_time; DaysElapsed++)
        {
            DateUpdate();
        }
