...
void sFLASH_Init(void)
{
  ...
  /*!< SPI configuration */
  #if 1//(SPI_FLASH_OP_MODE ==SPI_FLASH_CPU)
...
//	sect_clr(addr);   								  //É¾³ýÒ³		  
	...
	sFLASH_SendByte(0xad);
	sFLASH_SendByte((WriteAddr&0xffffff)>>16);
	sFLASH_SendByte((WriteAddr&0xffff)>>8);
	sFLASH_SendByte(WriteAddr&0xff);
  	sFLASH_SendByte(pBuffer[0]);
	sFLASH_SendByte(pBuffer[1]);
	sFLASH_CS_HIGH();
	i=2;
...
#else
#if(SPI_FLASH_OP_MODE ==SPI_FLASH_CPU)
