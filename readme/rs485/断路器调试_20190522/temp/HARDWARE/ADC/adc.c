...
{ 	
	...
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;//GPIO_Mode_AIN;		//模拟输入引脚
...
void Get_Adc6(u16 pAdcArray[])   
{
	...
	pAdcArray[0]=ADC_GetConversionValue(ADC1);	//返回最近一次ADC1规则组的转换结果
	//	ADC_SoftwareStartConvCmd(ADC1, ENABLE);		//使能指定的ADC1的软件转换启动功能	
	 
	//while(!ADC_GetFlagStatus(ADC1, ADC_FLAG_EOC ));//等待转换结束
	//pAdcArray[1]=ADC_GetConversionValue(ADC1);	//返回最近一次ADC1规则组的转换结果
}
